...
export interface Shape {
    ...
    rotation?: number;
    points?: Vector[];
    selectedPoint?: number /** index of selected point */;
    bitmap?: boolean;
    strokeCap?: 'but' | 'square' | 'round' /** only works when `bitmap` is set to `true` */;
    strokeJoin?: 'but' | 'square' | 'round' /** only works when `bitmap` is set to `true` */;
    strokeDash?: (number | Percentage)[] /** only works when `bitmap` is set to `true` */;
    ...
    disableAddPoints?: boolean;
    disableShowPoints?: boolean;
    disableAcceptSnap?: boolean;
...
export interface PinturaMarkupEditorProperties {
    ...
    /**
     * Will automatically select move tool when enabled, set to `true` to select for all tools, set to `false` to don't select, set to array of tool names to enable for specific tools only.
     */
    enableAutoSelectMoveTool?: boolean | string[];

    /**
     * Enable multi-select support, hold SHIFT to select multiple shapes
     */
...
export interface PinturaEditorBaseOptions {
    ...
    /** Called by Pintura when requesting a resource */
    willRequest?: (
        url: string,
...
export interface PinturaCropperDefaultOptions
    extends PinturaEditorBaseOptions,
        PinturaEditorImageOptions,
        CropPluginOptions {
    locale?: any;
...
export const getCropperDefaults: (
    options?: PinturaEditorDefaultOptions
) => PinturaCropperDefaultOptions;
...
export const createDefaultShapePreprocessor: (
    customProcessors?: [(shape: Shape, options?: { isPreview: boolean }) => Shape[]]
) => any;
...
export function removeNode(id: string, haystack: PinturaNode | PinturaNode[]): PinturaNode[];
 ...
 */
export const createMarkupEditorToolStyles: (
    toolStyles?:
        | {
...
interface StrokeColorPickerOptions extends ColorPickerOptions {
...
export const createMarkupEditorStrokeColorControl: (
    colors: ShapeColorOption[],
    options?: StrokeColorPickerOptions
) => ShapeControl;
